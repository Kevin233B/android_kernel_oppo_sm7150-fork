		...
		case USBPD_SVDM_DISCOVER_IDENTITY:
			...
			if (num_vdos == 3){
				...
				if(oppo_svid == OPPO_SVID){
					...
					if (handler) {
						usbpd_err(&pd->dev, "oppo_svid SVID: 0x%04x connect\n",
								handler->svid);
...
struct usbpd *usbpd_create(struct device *parent)
{
	...
	if (device_property_read_bool(parent, "qcom,msg_vconn_swap_reject"))
		pd->msg_vconn_swap_reject = true;
